...
- **一个窗口装下所有磁贴**。上一版是一磁贴一窗口，于是拖拽要跨进程同步，
  带来心跳、看门狗、指针捕获丢失一堆补丁。合并成一个窗口之后，那套补丁整体删掉了。
- **窗口区域即命中区域**。`SetWindowRgn` 把窗口裁成卡片圆角矩形的并集，
  "看得见"和"点得到"由同一份数据保证，不会出现看得见却点不着的错位。
- **卡片锚在显示器上，不是锚在窗口上**。窗口原点就是虚拟屏原点，它会随着
  显示器的增减而移动；只存窗口坐标的话，在主屏左边接一块屏，所有卡片第二天
  就集体偏了。详见 `lib\core\monitor.dart`。
- **启动幕布是原生分层窗口**，跑在自己的线程上，比 Flutter 引擎起得早，
  所以引擎初始化那几百毫秒里屏幕上也有东西可看。
- **日志是写给开发者的**。用户操作、程序反应、插件的每一次 HTTP 调用（含耗时与
  响应大小）都记，出问题时能直接指出是哪一环。

## 许可与致谢

MacroSTAR Studio 出品。天气数据来自 [Open-Meteo](https://open-meteo.com)，
歌词来自网易云与 [LRCLIB](https://lrclib.net)。
